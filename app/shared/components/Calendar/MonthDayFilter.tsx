...
export const MonthDayFilter = ({
  ...
  <>
    <div className="flex items-center justify-between gap-4">
      ...
      <span className="flex-shrink-0 text-xs text-gray-400 sm:text-sm">{countLabel}</span>
    </div>
    ...
    <div className="mt-4 flex items-center gap-2 overflow-x-auto scrollbar-hide">
      <button
        ...
        className={`flex-shrink-0 rounded-full border px-4 py-2 text-sm sm:px-5 sm:py-2.5 ${
      ...
      {days.map((day) => {
        ...
        return (
          <button
            ...
            className={`flex flex-shrink-0 items-center gap-1.5 rounded-full border px-3 py-2 text-sm sm:gap-2 sm:px-4 sm:py-2.5 ${
              isSelected
                ? 'border-transparent bg-green-200 font-semibold text-gray-900'
                : 'border-gray-200 bg-white text-gray-700'
            }`}
