  ...
  return (
    <>
      <div className="group relative mb-2 flex flex-col">
        ...
        {/* Fades in with the hover rather than blinking into place.
            `pointer-events-none` rather than `invisible` does the hiding:
            visibility cannot be transitioned, so it snapped on the way out. */}
        <div className="pointer-events-none absolute right-2 top-2 flex h-8 w-8 cursor-pointer items-center justify-center rounded-full bg-gray-800/50 p-1 opacity-0 transition-opacity duration-200 ease-out group-hover:pointer-events-auto group-hover:opacity-100 motion-reduce:transition-none">
          <BookmarkPlace
      ...
      <div className="flex flex-col items-start justify-start bg-white">
        <div className="mb-1 flex">
          <p className="text-xs font-bold text-gray-800 transition-colors duration-200 group-hover:text-primary">
            {place.title}
          </p>
        </div>
