...
import { MultiDayTicketModePicker } from '.';

describe('MultiDayTicketModePicker', () => {
  it('renders the question label', () => {
    const onChange = jest.fn();
    render(<MultiDayTicketModePicker value="entire-period" onChange={onChange} />);
  ...
  it('renders both radio options', () => {
    ...
    expect(screen.getByText('Create one ticket setup for the entire period')).toBeInTheDocument();
    expect(screen.getByText('Create tickets for each day separately')).toBeInTheDocument();
  });
  ...
  it('calls onChange when entire-period is selected', async () => {
    const user = userEvent.setup();
    const onChange = jest.fn();
    render(<MultiDayTicketModePicker value="each-day" onChange={onChange} />);
