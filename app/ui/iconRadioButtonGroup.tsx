...
export default function IconRadioButtonGroup() {
    ...
    return (
        <div className="border-[1px] rounded-[50px] border-gray-200 bg-white px-1 py-1 flex item-center">
            <div className={`grid grid-cols-2 gap-2`}>
                {options.map((option) => (
                    <div key={option.label}
                         onClick={() => setSelectedOption(option.label)}
                         className={clsx('cursor-pointer h-[35px] w-[35px] rounded-full flex justify-center items-center', {
